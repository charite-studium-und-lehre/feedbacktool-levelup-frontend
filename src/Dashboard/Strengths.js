...
import {compose, flow, overEvery, over, merge} from '../Utils/utils'

const loaded = overEvery([mcSelectors.loaded, ptmSelectors.loaded])
const load = () => over([mcActions.load(), ptmActions.load])
const ptmStrongestSubject = flow([ptmSelectors.getLatest, merge({}), ptmSelectors.strongestSubject])
...
const Strengths = compose([needsData(loaded, load), connect(stateToProps)])(({mcStrongestSubject, ptmStrongestSubject}) =>
    <div>
