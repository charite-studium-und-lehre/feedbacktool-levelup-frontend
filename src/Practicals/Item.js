    ...
    <div className={`p-2 pb-4 ${props.level === 3 ? 'ml-4' : props.level === 2 ? 'ml-3' : ''}`} style={{ backgroundColor: 'rgb(210, 220, 152)', boxShadow :' 2px 2px  10px rgb(3, 31, 56)'}}>
      <div className="row">
        ...
        <div className='col-lg-6' style={{ margin: props.level <= 2 ? '.5rem auto' : '' }}>
          <Score edit={!props.entry.entries.length} entryId={props.entryId} width={props.level === 1 ? '1rem' : props.level === 2 ? '.8rem' : '.8rem'} height={props.level === 1 ? '1rem' : props.level === 2 ? '.85rem' : '.6rem'} borderRadius={!!props.entry.entries.length ? '50%' : '5%'} />
        </div>
