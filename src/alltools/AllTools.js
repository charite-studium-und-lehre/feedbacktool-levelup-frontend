...
const  MakeStairsList = props => (
    <div className='col-12 col-lg-6'>
        <h2 className='pl-3'>{props.heading}</h2>
                 <div className='mt-4' >
            {props.data.map((d, i) => <div className='mt-3 pl-3'>
                <h5 className="font-weight-bold">{d.title}</h5>
                { i == 0  ? <Link to='/consulting' className="text-primary" >{d.href}</Link>
                :<a className="text-primary " key={i} href={d.href} target="blank">{d.href}</a>}
                </div>)}
            </div>
    </div>
)
